...
async def pub_(bot, message):
    ...
    sts = STS(frwd_id)
    if not sts.verify():
      await message.answer("You are clicking on one of my old button, please try again", show_alert=True)
      return await message.message.delete()
    i = sts.get(full=True)
    if i.TO in temp.IS_FRWD_CHAT:
      return await message.answer("In Target chat a task is in progress. Please wait until task completes", show_alert=True)
    ...
    try:
       ...
       await msg_edit(m, f"**Please make your [UserBot / Bot](t.me/{_bot['username']}) admin In target channel with fulll permissions**", retry_btn(frwd_id), True)
       return await stop(client, user)
    temp.forwardings += 1
    await db.add_frwd(user)
    await send(client, user, "<b>🧡 ғᴏʀᴡᴀʀᴅɪɴɢ sᴛᴀʀᴛᴇᴅ 🥀 <a href=https://t.me/KingMogger>ᴅᴇᴠ</a>🥀</b>")
    sts.add(time=True)
    ...
    if locked:
        try:
          ...
          async for message in client.iter_messages(
            ...
            return await stop(client, user)
        temp.IS_FRWD_CHAT.remove(sts.TO)
        await send(client, user, "<b>🎉 ғᴏʀᴡᴀᴅɪɴɢ ᴄᴏᴍᴘʟᴇᴛᴇᴅ 🥀 <a href=https://t.me/KingMogger>ᴅᴇᴠ</a>🥀</b>")
...
async def edit(msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append(
         [InlineKeyboardButton('sᴜᴘᴘᴏʀᴛ 🚸', url='https://t.me/KingMogger'),
         InlineKeyboardButton('ᴜᴘᴅᴀᴛᴇs ✅', url='https://t.me/KingMogger')]
         )
   else:
...
def retry_btn(id):
    return InlineKeyboardMarkup([[InlineKeyboardButton('♻️ʀᴇᴛʀʏ ♻️', f"start_public_{id}")]])
